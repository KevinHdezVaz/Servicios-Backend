...
if($status == 'accept')
{
	...
	$con->query("update orders set status='procesando',a_status=2,r_status='Accepted' where id=".$oid."");
	$con->query("update rider set accept = accept+1 where id=".$rid."");
	$checks = $con->query("select * from orders where id=".$oid."")->fetch_assoc();
		$content = array(
"en" => 'Tu agua se entregara en un rato.'//mesaj burasi
);
$fields = array(
'app_id' => ONE_KEY,
...
else if($status == 'cancle')
{
	...
	$checks = $con->query("select * from orders where id=".$oid."")->fetch_assoc();
		$content = array(
"en" => 'Te cancelaron parce'//mesaj burasi
);
$fields = array(
'app_id' => ONE_KEY,
'included_segments' =>  array('All'), // Enviar a todos los usuarios
...
{
	$sign = $data['sign'];
	$con->query("update orders set a_status=3,r_status='Delivered',status='entregado',photo='".$sign."' where id=".$oid."");
	$con->query("update rider set complete = complete + 1 where id=".$rid."");
    $checks = $con->query("select * from orders where id=".$oid."")->fetch_assoc();
	$uid = $checks['uid'];
	$checkcomplete = $con->query("select * from orders where uid=".$uid." and status='entregado'")->num_rows;
	if($checkcomplete == 1)
	{
	...
	if($refercode != '')
	{
		...
		$content = array(
"en" => 'Tu producto se ha entregado satisfactoriamente'//mesaj burasi
);
$fields = array(
'app_id' => ONE_KEY,
'included_segments' =>  array('All'), // Enviar a todos los usuarios
