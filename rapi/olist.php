...
if ($data['rid'] == '') { 
    ...
    $sel = $con->query("select * from orders where rid=" . $rid . " and status != 'entregado' and status != 'cancelled' and p_method != 'Pickup Myself' order by id desc");
    ...
    if ($sel->num_rows != 0) {
        ...
        while ($row = $sel->fetch_assoc()) {
            ...
            for ($i = 0; $i < count($a); $i++) {
                ...
                $k[$i] = array(
                    "product_name" => $a[$i],
                    "product_price" => number_format((float)$b[$i], 2, '.', ''),
                    "product_weight" => $c[$i],
                    "product_qty" => $d[$i],
                    "product_image" => $getimage['pimg'],
                    "discount" => $getimage['discount']
                );
            }
            
            $result['productinfo'] = $k;
            
            if ($row['p_method'] == 'Pickup myself' && $row['status'] != 'entregado' && $row['status'] != 'cancelled') {
                $status = $row['p_method'];
            } else {
                $status = $row['status'];
            }
