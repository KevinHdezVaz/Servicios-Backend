                  ...
                  ?>
                          ...
                          <div class="form-body">
                            ...
                            <div class="form-group">
                              ...
                              <select name="srider" class="form-control">
                                <?php //busca un repartidor disponible

                                $rid = $con->query("select * from rider where a_status=1 and status=1");
                                while ($ro = $rid->fetch_assoc()) {
                                  //coloca en el option el repartidor que conincide con la condicion
                                ?>
                    ...
                    if (isset($_POST['as_order'])) {
                      ...
                      if ($check['r_status'] != 'Accepted') {
                        ...
                        $con->query("update orders set rid=" . $rid . ",pickup='" . $pickup . "',a_status=1,r_status='asignado prro' where id=" . $id . "");
                        $con->query("insert into rnoti(`rid`,`msg`,`date`)values(" . $rid . ",'Nuevo pedido de agua','" . $timestamp . "')");
                        $content = array(
                        ...
                        // Verificar la respuesta de OneSignal
                        if ($response === false) {
                      ...
                      <div class="card-block card-dashboard">
                        ...
                        <table class="table table-striped" id="example">
                          ...
                          <tbody>
                            ...
                            <?php
                            $sel = $con->query("select * from orders where status !='entregado' order by id desc");
                            ...
                            ?>
                              <tr>
                                ...
                                <td>
                                  ...
                                  <?php if ($row['p_method'] == 'Pickup Myself' and $row['status'] != 'entregado' and $row['status'] != 'cancelled') { ?>
                                    <a href="?status=entregado&id=<?php echo $row['id']; ?>"><button class="btn shadow-z-2 btn-success">Make Completed</button></a>
                                  <?php } ?>
